...
//------------------------------------------------------------------------------
    ...
    auto init()
    {
        ...
        if (-1 == (_client_sock = socket(AF_UNIX, SOCK_STREAM, 0))) return Error::client_socket_failed;
        ...
        {
            close(_client_sock);
            return Error::client_bind_failed;
        }
        ...
        {
            close(_client_sock);
            return Error::client_connect_failed;
        }
...
//------------------------------------------------------------------------------
    ...
    Error::error send_all(int sockaddr, const char* buf, int len)
    {
        auto total = 0;
        auto bytesleft = len;
        auto n = 0;
        ...
        return n == -1 ? Error::client_send_failed : Error::no_error;
    }
...
//------------------------------------------------------------------------------
    ...
    auto request(std::string request_string)
    {
        ...
        if (Error::success(result))
            ...
            {
                close(_client_sock);
                result = Error::client_recv_failed;
            }
